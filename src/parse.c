...
int	validate_julia_params(char **argv, t_fractal *f)
{
	...
	if (argv[4] && is_valid_integer(argv[4]))
		f->max_iter = ft_atoi(argv[4]);
	else
		f->max_iter = 1;
	if (f->c_julia_real < JULIA_REAL_MIN || f->c_julia_imag < JULIA_IMAG_MIN
		|| f->c_julia_imag > JULIA_IMAG_MAX || f->c_julia_real > JULIA_REAL_MAX)
	{
...
int	set_fractal_parameters(int argc, char **argv, t_fractal *f)
{
	...
	if (f->name[0] == 'J')
	{
		if (argc < 4 || argc > 5 || !validate_julia_params(argv, f))
			// Cambié el check de argc
			return (0);
	}
	else
	{
		...
		if (argc == 3 && ft_atoi(argv[2]) && ft_atoi(argv[2]) > 1)
